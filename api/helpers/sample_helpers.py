...
def handle_parse_vcf_results_for_sample(variant_list, sample_id):
    ...
    # Add all variants to the sample in a single operation
    sample_small_variants.variants.add(
        *SmallVariant.objects.filter(variant_id__in=variant_ids)
    )

    # Create the SampleSmallVariantData objects
    data_to_create = []
    for vid in variant_map.keys():
