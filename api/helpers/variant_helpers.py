...
def get_non_annotated_variants(variant_list) -> list:
    ...
    snv_ids = VariantAnnotation.objects.filter(variant_id__in=variant_ids, is_annotated = True).values_list(
...
def create_snv(variant_id: str) -> VariantAnnotation:

    snv = VariantAnnotation.objects.get_or_create(variant_id=variant_id)[0]
    return snv


def update_snv(variant_dict: dict) -> VariantAnnotation:
    snv, _ = VariantAnnotation.objects.get_or_create(variant_id=variant_dict["variant_id"])
    
    snv_attributes = VariantAnnotation.__dict__.keys()
