...
user_creation_total = Counter(
    "cosap_user_creation_total", "Total number of users created"
)
...
project_deletion_total = Counter(
    "cosap_project_deletion_total", "Total number of projects deleted"
)
...
file_deletion_total = Counter(
    "cosap_file_deletion_total", "Total number of files deleted"
)
...
cosap_active_workers = Gauge(
    "cosap_active_workers", "Number of active Celery workers", ["worker_type"]
)

# Current state metrics (perfect for Grafana gauges)
active_projects_count = Gauge(
    "cosap_active_projects_count", "Current number of active projects", ["project_type"]
)

active_users_count = Gauge(
    "cosap_active_users_count", "Current number of active users (logged in last 24h)"
)

pending_jobs_count = Gauge(
    "cosap_pending_jobs_count", "Current number of pending COSAP jobs", ["job_type"]
)

# Report and analysis metrics
report_creation_total = Counter(
    "cosap_report_creation_total", "Total number of reports created", ["report_type"]
)
