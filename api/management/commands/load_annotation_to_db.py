...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        existing_variants = {
            v.variant_id: v
            for v in SmallVariant.objects.filter(variant_id__in=variant_ids)
        }
        ...
        variant_annotation_fields = {
            field.name for field in VariantAnnotation._meta.fields
        }
        ...
        for variant in data:
            ...
            annotations_to_create.append(
                VariantAnnotation(variant=svar, **cleaned_variant)
            )
